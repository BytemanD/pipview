    ...
    <v-navigation-drawer permanent expand-on-hover width="180">
      ...
      <v-list density="compact" nav>
        <v-list-item v-for="item in navItems" :key="item.value" :prepend-icon="item.icon" :title="item.title"
          :value="item.value" :active="currentTab === item.value" @click="currentTab = item.value"
      ...
      <template v-slot:append>
        <v-divider></v-divider>
        <v-list density="compact" nav>
          <v-list-item :prepend-icon="activeTasks.length > 0 ? 'mdi-progress-clock' : 'mdi-timer-sand'
            " :title="activeTasks.length > 0 ? `任务 (${activeTasks.length})` : '无任务'
              " :value="'tasks'" :active="currentTab === 'tasks'" @click="currentTab = 'tasks'"
            color="primary"></v-list-item>
        </v-list>
      </template>
    </v-navigation-drawer>
