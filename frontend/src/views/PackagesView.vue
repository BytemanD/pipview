  ...
  <div>
        ...
        <v-card class="pa-3" :class="{ 'cursor-pointer': stat.clickable }" @click="stat.onClick && stat.onClick()">
          <div class="d-flex align-center justify-space-between">
            <div class="d-flex align-center flex-grow-1">
              <v-icon :color="stat.color" size="24" class="mr-3">{{
                stat.icon
                }}</v-icon>
              <div>
    ...
    <v-card>
      ...
      <v-data-table v-model="selectedPackages" show-select :headers="headers" :items="filteredPackages"
        ...
        <template v-slot:item.latest_version="{ item }">
          ...
          <v-chip v-else-if="
            item.latest_version && item.latest_version !== item.version
        ...
        <v-card-text>
          ...
          <v-window v-model="installTab" class="mt-4">
            ...
            <v-window-item value="local" class="pt-3">
              ...
              <div v-if="localFileName" class="text-caption text-medium-emphasis">
                已选择: {{ localFileName }}
              </div>
            </v-window-item>
          </v-window>
    ...
    <v-dialog v-model="showVersionDialogFlag" max-width="500" scrollable>
      <v-card>
        ...
        <v-card-text>
          <div class="mb-3">
            当前版本: <strong>{{ currentPackage?.version }}</strong>
          </div>
      ...
      <v-card :title="packageDetail?.name" :subtitle="packageDetail?.summary">
        ...
        <v-card-text v-else-if="packageDetail">
          <v-list density="compact">
            ...
            <v-list-item v-if="
              packageDetail.classifiers && packageDetail.classifiers.length
            ">
    ...
    <v-dialog v-model="showUninstallDialog" max-width="400" scrollable>
      <v-card>
        <v-card-title class="text-error">确定要卸载
          <strong>{{ uninstallTarget?.name }}</strong> 吗？</v-card-title>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="showBatchUninstallDialog" max-width="400" scrollable>
      <v-card>
        <v-card-title class="text-error">
          确定要卸载以下
          <strong>{{ selectedPackages.length }}</strong> 个包吗？<br />
        </v-card-title>
        <v-card-text>
...
const statCards = computed(() => [
  {
    label: "pip",
    value: pipInstalled.value ? pipVersion.value || "-" : "未安装",
    icon: "mdi-toolbox",
    color: pipInstalled.value ? "primary" : "error",
  },
  {
    label: "可升级",
    value: statsUpgradable.value,
    icon: "mdi-star",
    color: "success",
  },
  {
  ...
  {
    label: "依赖状态",
    value: hasConflict.value ? "冲突" : "正常",
    icon: hasConflict.value ? "mdi-alert" : "mdi-shield-check",
    color: hasConflict.value ? "error" : "success",
    clickable: hasConflict.value,
    onClick: hasConflict.value
...
const confirmDowngrade = async () => {
  ...
  const res = await packagesApi.downgrade(
