...
export const packagesApi = {
  list: (search = "") => apiCall(`/packages?search=${search}`),

  get: (packageName) => apiCall(`/packages/${packageName}`),
  ...
  versions: (packageName) => apiCall(`/packages/${packageName}/versions`),

  dependencies: (packageName) =>
    apiCall(`/packages/${packageName}/dependencies`),

  latestVersion: (packageName) => apiCall(`/packages/search?q=${packageName}`),

  checkConflicts: () => apiCall("/packages/conflicts"),

  getCache: () => apiCall("/packages/cache"),

  clearCache: () => apiCall("/packages/cache", { method: "DELETE" }),
  ...
  checkUpdates: () => apiCall("/packages/updates"),
};
...
export const sourcesApi = {
  ...
  reset: () => apiCall("/sources/current", { method: "DELETE" }),
};
...
export const tasksApi = {
  ...
  output: (taskId) => apiCall(`/tasks/${taskId}/output`),

  cancel: (taskId) => apiCall(`/tasks/${taskId}`, { method: "DELETE" }),
};
